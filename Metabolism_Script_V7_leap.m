...
% *************************************************************************
% -- PARAMETERS > Imedea
ik660choice=1; % 1>Original formulation by Cole&Caraco 98 2> Wanninkhof 2014 3> K&K Cubic 4>Dueñas
daylightRoption=2; % 1> Using Night Resp. for daylight 2> Using Day R as a function of Temp. 3> Using Day R + night anomalies
...
% > Imedea: Include the choice of different k660 formulations from K&K 2010
    k600_calc=windToK600_imedea(winddata,SDdate,windheight,ik660choice); %use windToK600_imedea function to calculate k600
else
...
% get true O2 concentration in mg/L. ¡We've got those concentrations!
...
% -> Imedea : New equation to include salinity ¿De dónde sale esto?
...
for i=firstdeploy:lastdeploy
   ...
   SDfecha=datenum(SDYear,1,0)+SDDOY;
Fechas=SDfecha(first_row:last_row);
fechas=unique(Fechas,'rows');
...
%    schmidt=(1800.6-120.1*temp)+(3.7818*temp.^2)-0.047608*temp.^3;
  ...
  % Wanninkhof (2014):
%schmidt=(1920.4-135.6*temp)+5.212*temp.^2-0.10939*temp.^3+0.00093777*temp.^4;
  % Dobashi & Ho (2023)- Temperature and salinity as variables:
schmidt= 1733.6+5.1437.*sal+(-121.69-0.29994.*sal).*temp+(4.5556+0.0099283.*sal).*temp.^2+(-0.091963-0.00018688.*sal).*temp.^3+(0.00075576+1.4784.*10.^-6.*sal).*temp.^4;
   ...
   O2sat=O2sat(SDtemp, sal); % umol/kg 
   dens=density(sal,SDtemp)/1000; %kg/m3
   O2sat=O2sat.*dens/1000; %mmol/m3      
...
for xday=min(floor(time)):max(floor(time)) %Days for which we have data ! IMEDEA
    ...
    kdatesunrise=floor(xday)+(hsunrise(j+1)-1)/24;
      ...
      i_daylighthours=find(time>=kdatesunrise & time<=kdatesunset); 
            %For each day, find the rows (i_daylighthours) that correspond to data obtained
...
metab = [DOY GPP R24 NEPlight dailyzmix(:,2) kO2];
metab = excise(metab);
...
OUTPUT=[fechas DOY GPP R24 NEPlight mean24htemp kO2];
OUTPUT = excise(OUTPUT);
OUTPUT2=[fechas DOY R24 R24day R24night mean24htemp meandaytemp meannighttemp daylight];
OUTPUT2 = excise(OUTPUT2);
OUTPUT3=[M_all];
OUTPUT3 = excise(OUTPUT3);
if savefile==1
    ...
    fprintf(fid,'%s\n','fechas, DOY, GPP, R24, NEP, Mean24hTemp, k02');
    ...
    savedfilename=[savepathname savefilename];
    fid=fopen(savedfilename,'w'); %open file for writing
    fprintf(fid,'%s\n','fechas, DOY, R24, Rday, Rnight, Mean24hTemp, MeanDayTemp,MeanNightTemp, Hoursofdaylight ');
    ...
    savedfilename=[savepathname savefilename];
    fid=fopen(savedfilename,'w'); %open file for writing
    fprintf(fid,'%s\n','DOY, fechas, T, metabolism, kO2, gasflux, zmix');
    fprintf(fid, '%f,%f,%f,%f,%f,%f,%f\n',OUTPUT3');
    fclose(fid);
    disp(' ');disp(['File saved as:', savedfilename]);
end
...
if showfigures==1  
    ...
    hold on;
    plot(DOY, NEPlight,'-s','MarkerSize',10,'MarkerEdgeColor','Black','MarkerFaceColor', rgb ('LightSkyBlue'), 'Color', ...
...
DO=sondedata(:,5);
figure;
plot(SDdate,DO, 'Linewidth', 1, 'Color', rgb ('DarkBlue')); dateaxis('x',17);ylabel('(O_2 Saturation %)',FontSize=12);
ylim([65 145]);...
